...
export default function UploadModal({ existingPhotoNames, onClose, onUploaded }: Props) {
  ...
  const statusIcon = (e: FileEntry) => {
    ...
    if (e.status === 'error' || e.status === 'skipped') return <span className="text-gray-400 text-lg">–</span>;
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
      ...
      <div className="slide-up relative bg-white rounded-3xl shadow-2xl w-full max-w-xl max-h-[90vh] flex flex-col overflow-hidden">
        {/* Header */}
        <div className="px-6 pt-6 pb-4 border-b border-gray-100 flex-shrink-0">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-bold text-gray-900 tracking-tight">Tải ảnh lên</h2>
              <p className="text-sm text-gray-500 mt-0.5">JPG, PNG, WEBP, HEIC · Tối đa 10MB/ảnh</p>
            </div>
            {!isUploading && (
              <button onClick={onClose} className="w-9 h-9 flex items-center justify-center rounded-xl text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors text-xl">×</button>
            )}
          </div>
        </div>
        ...
        {!done && (
          ...
          >
            ...
            <p className="text-sm text-gray-500">Kéo thả ảnh vào đây</p>
              ...
              <div key={entry.id} className="flex items-start gap-3 p-3 rounded-xl bg-gray-50 border border-gray-100">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    {!isUploading && !done && (
                      <button onClick={() => removeEntry(entry.id)} className="text-gray-300 hover:text-gray-500 flex-shrink-0 text-base leading-none">×</button>
                    )}
                    {statusIcon(entry)}
                  </div>
                  <p className="text-xs text-gray-400 mt-0.5">{formatFileSize(entry.file.size)}</p>
                      ...
                      <div className="flex gap-1.5">
                        {(['skip', 'rename', 'replace'] as const).map((action) => (
                          <button
                            ...
                            className="px-2.5 py-1 rounded-lg text-xs font-medium border transition-colors bg-white border-gray-200 text-gray-600 hover:border-blue-400 hover:text-blue-600"
                          >
        ...
        {done && (
          <div className="flex-1 flex flex-col items-center justify-center py-8 gap-3">
            ...
            <p className="text-lg font-semibold text-gray-900">Tải lên hoàn tất!</p>
        ...
        <div className="flex-shrink-0 px-6 py-4 border-t border-gray-100 flex items-center justify-between gap-3">
          ...
          <div className="flex gap-3">
            {done ? (
              ...
              <>
                <button onClick={onClose} disabled={isUploading} className="px-4 py-2.5 rounded-xl border border-gray-200 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors disabled:opacity-40">
                  Hủy
                </button>
                <button
                  onClick={startUpload}
