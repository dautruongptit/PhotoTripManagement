...
export default function UpgradeModal({ usedBytes, limitBytes, onClose, onSelectPlan }: Props) {
  ...
  const handleConfirm = () => {
    setConfirming(true);
    // Mô phỏng gọi API thanh toán/nâng cấp — thay bằng gọi backend thật khi có cổng thanh toán
    setTimeout(() => {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={!confirming ? onClose : undefined} />
      <div className="slide-up relative bg-white dark:bg-gray-900 rounded-3xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="px-6 pt-6 pb-4 border-b border-gray-100 dark:border-gray-800 flex-shrink-0">
          <div className="flex items-center justify-between">
            ...
            <button onClick={onClose} disabled={confirming} className="w-9 h-9 flex items-center justify-center rounded-xl text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors text-xl disabled:opacity-40">×</button>
          </div>
        ...
        <div className="flex-shrink-0 px-6 py-4 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between gap-3">
          ...
          <div className="flex gap-3">
            <button onClick={onClose} disabled={confirming} className="px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors disabled:opacity-40">
              Để sau
            </button>
            <button
              onClick={handleConfirm}
              disabled={confirming}
              className="px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-sm font-semibold text-white transition-all shadow-sm hover:shadow-md hover:shadow-blue-200 disabled:opacity-60 flex items-center gap-2"
            >
              {confirming && (
              ...
              {confirming ? 'Đang xử lý…' : `Nâng cấp ${selectedPlan.label}`}
            </button>
          </div>
        </div>
      </div>
